...
def test_build_bundle_writes_merged_consumer_contract(tmp_path):
    ...
    assert summary["counts"] == {
        ...
        "source_count": 39,
        "source_package_count": 116,
        "warning_count": 1,
    }
    assert len(rows) == 135430
    ...
    assert source_packages["source_package_count"] == 116
    ...
    assert coverage["fact_count"] == 135430
    ...
    assert coverage["counts"]["by_period"] == {
        ...
        "calendar_year:2021": 3924,
        "calendar_year:2022": 1845,
        "calendar_year:2023": 3703,
        "calendar_year:2024": 28120,
        "calendar_year:2025": 4372,
    ...
    assert coverage["counts"]["by_geography"]["country:K02000001"] == 3821
    assert coverage["counts"]["by_geography"]["country:K03000001"] == 277
    assert len(coverage["counts"]["by_geography"]) == 9390
