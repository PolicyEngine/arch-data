"""Consumer artifacts and period-contract resolution for Chronicle profiles.

This module is the supported way for downstream builds (Microcosm, Thesis,
future rule engines) to consume Chronicle facts: a versioned on-disk artifact
...
failure mode that produced PolicyEngine/microcosm#212 (SOI tax-year levels
...
:class:`PeriodAlignmentDeclaration`. Chronicle records the declaration in the
...
from policyengine_chronicle.target_profiles.model import (
...
class PeriodAlignmentDeclaration:
    ...
    value outside Chronicle. It carries no values: parameters reference factor
...
class ResolvedTarget:
    """One profile target row resolved against a Chronicle fact row.
    ...
    transform the value to ``requested_period`` with ``alignment``; Chronicle
...
def _select_rows(
    ...
    *,
    ...
    issues: list[ResolutionIssue] = []
    selector = dict(target.chronicle_selector)
    ...
    matched = [
        ...
        and all(_selector_matches(row, key, value) for key, value in selector.items())
    ]
...
def _selector_matches(row: Mapping[str, Any], key: str, value: Any) -> bool:
    if key == "dimensions":
        ...
        if isinstance(value, Mapping):
            return all(
                dimensions.get(name) == expected for name, expected in value.items()
            )
        if isinstance(value, list):
            # Dimension-identity selectors match order-insensitively on the
            # exact set of dimension variable names the row carries.
            return sorted(dimensions) == sorted(value)
        return False
...
class ConsumerArtifact:
    """A loaded Chronicle consumer artifact.
...
def _load_profiles(
    ...
    for profile_id in profile_ids:
        ...
        payload = json.loads(
            _resource_files("policyengine_chronicle.target_profiles")
